...
This project utilizes various trigram-based algorithms to generate jazz melodies, trained on a transcription of Oscar Peterson's "C Jam Blues".
...
*   **`Main.java`**: The entry point. Handles parsing input, running different generation algorithms, and saving results.
...
## `sample-output/`
Contains MP3 files for melodies generated by each of the algorithms, as well as for the input melody in `melody.mp3`. These were created from the generated MIDI files. 
...
*   **Average options for third note**: A measure of unpredictability. Higher values mean the model has more choices on average.
...
```
For each bigram in the frequency map, there are an average of 1.37 unique next notes in the map. This means that the output of this algorithm is very predictable compared to the others.
...
*   **Cons**: Can produce awkward intervals if the chord progression is not standard.
*   **Stats**:
    *   **Theoretical Calculation**:
...
*   **Results**: This algorithm produces melodies that fit the chord changes well but may lack the specific melodic contour of the original.
...
*   **Pros**: Can generate very coherent rhythmic structures independent of pitch constraints.
...
*   **Pros**: Combines the rhythmic coherence of the separated model with the smoother voice leading of the octave-ignorant approach.
...
*   **Results**: Smoother than the previous algorithm but still less coherent than the standard algorithm.
